...
function MBMultiCell($w,$h,$txt,$border=0,$align='L',$fill=0)
{
  ...
  while($i<$nb)
  {
    ...
    $prev_ascii=$ascii;
    $ascii = (ord($c) < 128 || ( ord($c) > 160 && ord($c) < 224));
    $hwkana = ($japanese && ( ord($c) > 160 && ord($c) < 224 ));
    if($c=="\n")
    {
      //Explicit line break
      if($this->ws>0)
    ...
    $l += ($ascii && isset($cw[$c])) ? $cw[$c] : ($hwkana ? 500 : 1000);
    if($l>$wmax)
    {
...
function conv($data_){
    if(empty($data_)){
        ...
        return mb_convert_encoding($data_, 'sjis-win', 'utf-8') ;
    }
}
...
function ht2br($_data, $_cont = null, $_attr = null, $_decimal = null) {
	...
	if( $_attr === 'AMOUNT' || $_attr === 'SUBTOTAL' || $_attr === 'SALES_TAX' || $_attr === 'TOTAL'){
