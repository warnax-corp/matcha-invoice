...
<!-- contents_Start -->
<div id="contents">
  <?php echo $form->create("Totalbill", array("type" => "post", "controller" => "totalbills", "class"=>"Totalbill", 'action' => 'edit')); ?>
          ...
          <?php echo $form->text('NO' ,array('class' => 'w180 p2'.($form->error('NO')?' error':''),'maxlength'=>20)); ?>
					...
					<td width="320">
						<script language=JavaScript>
							<!--
						    var cal1 = new JKL.Calendar("calid", "TotalbillEditForm", "data[Totalbill][DATE]");
						  //-->
					  </script>
			...
			<?php
				if(isset($billlist)&&is_array($billlist)){
					...
					foreach($billlist as $key => $val){
						...
						<td id=TOTAL".$i.">".($val['Bill']['TOTAL']?$customHtml->ht2br($val['Bill']['TOTAL'],'Totalbill','TOTAL'):'　')."</td>";
			...
			<div class="list_area">
				<?php
						...
						<tr>';
							...
							<td>".$form->input('CARRY_BILL', array('maxlength'=>'15','readonly'=>'readonly','error'=>false,'label' => false, 'div' => false, 'class' => 'w130'.($form->error('CARRY_BILL')?' error':'')))."</td>
							<td>".$form->input('SALE', array('maxlength'=>'15','error'=>false,'readonly'=>'readonly','label' => false, 'div' => false,'class' => 'w130'.($form->error('SALE')?' error':'')))."</td>
							<td>".$form->input('SALE_TAX', array('maxlength'=>'15','error'=>false,'readonly'=>'readonly','label' => false, 'div' => false,'class' => 'w130'.($form->error('SALE_TAX')?' error':'')))."</td>
							<td>".$form->input('THISM_BILL', array('maxlength'=>'15','readonly'=>'readonly','error'=>false,'label' => false, 'div' => false, 'class' => 'w130'.($form->error('THISM_BILL')?' error':'')))."</td>
							</tr>";
				?>
				</table>
				<div>
		...
		<div class="contents_area">
			...
			<tr>
				...
				<td style="width:730px;">
					<?php echo $form->input('SUBTOTAL', array('error'=>false,'maxlength'=>'15','label' => false, 'readonly'=>'readonly','div' => false,'class' => 'w300'.($form->error('SUBTOTAL')?' error':'')));?>
			...
			<tr>
				<th>消費税</th>
				<td>
					<?php echo $form->input('SALE_TAX', array('error'=>false,'maxlength'=>'15','label' => false, 'readonly'=>'readonly','div' => false,'class' => 'w300'.($form->error('SALE_TAX')?' error':'')));?>
			...
			<tr>
				<th>合計請求額</th>
				<td>
					<?php echo $form->input('THISM_BILL', array('error'=>false,'maxlength'=>'15','label' => false,'readonly'=>'readonly','div' => false, 'class' => 'w300'.($form->error('THISM_BILL')?' error':'')))?>
	...
	<div class="edit_btn">
		...
		<?php echo $form->hidden('TBL_ID',array('value'=>$tbl_id));?>
