...
echo $form->create("Delivery", array('type' => 'get', 'action' => 'index'));
?>
<div id="contents">
                    ...
                    <table width="940" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr><th>発行日 開始日</th>
                            <td width="320">
                                <script language=JavaScript>
                                    <!--
                                    var cal1 = new JKL.Calendar("calid", "DeliveryIndexForm", "ACTION_DATE_FROM");
                                  //-->
                              </script>
                        ...
                        <th>発行日 終了日</th>
                            <td width="320">
                                <script language=JavaScript>
                                    <!--
                                    var cal2 = new JKL.Calendar("calid", "DeliveryIndexForm", "ACTION_DATE_TO");
                                  //-->
                              </script>
...
<?php echo $form->end(); ?>
	...
	<?php echo $html->link($html->image("bt_new.jpg"), array('controller' => 'deliveries',   'action' => 'add'),array('escape' => false),null,false); ?>
	<?php echo $html->link($html->image("bt_excel.jpg"), array('controller' => 'deliveries',   'action' => 'export'),array('escape' => false),null,false); ?>
	</div>


	<h3><div class="edit_02_deliver"><span class="edit_txt">&nbsp;</span></div></h3>

	<div class="contents_box mb40">
		<div id='pagination'>
		<?php echo $paginator->data['Delivery']["count"]; ?>
		</div>
		...
		<div class="list_area">
			...
			<?php echo $form->create("Delivery", array('type' => 'post', 'action' => 'action')); ?>
				<table width="900" cellpadding="0" cellspacing="0" border="0" id="index_table">
					...
					<tr>
						...
						<th class="w50"><?php echo $customHtml->sortLink('No.', 'Delivery.MDV_ID'); ?></th>
						<th class="w100"><?php echo $customHtml->sortLink('顧客名', 'Customer.NAME_KANA'); ?></th>
						<th class="w150"><?php echo $customHtml->sortLink('件名', 'Delivery.SUBJECT'); ?></th>
						<th class="w100"><?php echo $customHtml->sortLink('合計金額', 'Delivery.CAST_TOTAL'); ?></th>
						<th class="w100"><?php echo $customHtml->sortLink('発行日', 'Delivery.ISSUE_DATE'); ?></th>
						<?php echo($user['AUTHORITY']!=1)?'<th class="w150">'.$customHtml->sortLink('作成者', 'Delivery.USR_ID').' / ':'';?>
						<?php echo($user['AUTHORITY']!=1)?$customHtml->sortLink('更新者', 'Delivery.UPDATE_USR_ID').'</th>':'';?>
						<th class="w100"><?php echo $customHtml->sortLink('発行ステータス', 'Delivery.STATUS'); ?></th>
						<th class="w100">メモ</th>
					</tr>
					</thead>
					...
					<tr>
						<td><?php echo $form->checkbox($val['Delivery']['MDV_ID'], array('class'=> 'chk')); ?></td>
					<?php echo  (isset($authcheck[$key]))?'<div class=auth'.$val["Delivery"]["MDV_ID"].' style=display:none;>'.$authcheck[$key].'</div>':''; ?>						<td><?php echo $customHtml->ht2br($val['Delivery']['MDV_ID'],'Delivery','MDV_ID'); ?></td>
						<td><?php echo $customHtml->ht2br($val['Customer']['NAME'],'Customer','NAME'); ?></td>
						<td><?php echo $html->link($val['Delivery']['SUBJECT'], array('controller' => 'deliveries', 'action' => 'check/'.$val['Delivery']['MDV_ID'])); ?></td>
						<td><?php echo isset($val['Delivery']['TOTAL'])?$customHtml->ht2br($val['Delivery']['TOTAL'], 'Delivery', 'TOTAL').'円':'&nbsp'; ?></td>
						<td><?php echo $val['Delivery']['ISSUE_DATE']?$val['Delivery']['ISSUE_DATE']:'&nbsp'; ?></td>
						<?php 	echo($user['AUTHORITY']!=1)?'<td>'.$customHtml->ht2br($val["User"]["NAME"],"Delivery","NAME").' / ':''?>
					<?php 	echo($user['AUTHORITY']!=1)?($val["UpdateUser"]["NAME"]==NULL)?'&nbsp;</td>':$customHtml->ht2br($val["UpdateUser"]["NAME"],"Delivery","NAME").'</td>':''?></td>
						<td><?php echo $status[$val['Delivery']['STATUS']]; ?></td>
						<td><?php echo $val['Delivery']['MEMO']? $customHtml->ht2br($val['Delivery']['MEMO'],'Delivery','MEMO'):'&nbsp'; ?></td>
					</tr>
					<?php endforeach; ?>
					</tbody>
			<?php endif; ?>
