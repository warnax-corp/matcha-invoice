...
$(document).ready(function($){
	if(<?php echo $collapse_other?'0':'1';?>) {
		$('div.contents_area4').slideToggle();
		$('span.show_bt3_on').show();
		$('span.show_bt3_off').hide();
	}
	if(<?php echo $collapse_management?'0':'1';?>) {
		$('div.contents_area5').slideToggle();
		$('span.show_bt4_on').show();
		$('span.show_bt4_off').hide();
	}
	...
	$(".row_<?php echo $i; ?>").hover(
			function (){
				$(':text[name*="data[<?php echo $i; ?>]"]').addClass('hoverLine');
			},
			function (){
				$(':text[name*="data[<?php echo $i; ?>]"]').removeClass('hoverLine');
			}
	);
	<?php }?>
